...
def login_user_api(username, password):
    return _safe_post(f"{API_BASE_URL}/auth/login", {"username": username, "password": password}, timeout=10)
...
def get_messages(role=None, department=None, limit=50, unread_only=False, include_archived=False, sender_name=None):
    params = {
        "limit": int(limit),
...
def acknowledge_message_api(message_id):
    return _safe_post(f"{API_BASE_URL}/messages/ack", payload={"message_id": message_id}, timeout=20)


def archive_message_api(message_id):
    return _safe_post(f"{API_BASE_URL}/messages/archive", payload={"message_id": message_id}, timeout=20)